...
public enum IndirectCollaboratorModelProvider {
	INSTANCE;
	...
	private IndirectCollaboratorModelProvider() {
		...
		{
			// create thread here
			InDirectCollaboratorThread myThread = new InDirectCollaboratorThread();
			myThread.start();
		}
	...
	public void getIndirectCollaboratorsFromServlet() {

		// invoke servlet //pass the collabName
		HttpGet httpget = new HttpGet("http://" + ipAddTomcat
				+ "/collabserver/EInCServlet?cName=" + collabName);
	...
	class InDirectCollaboratorThread extends Thread {
			...
			while (true) {
				...
				try {
					...
					if (collabClient != null) {
						collaborators.clear();
						System.out
								.println("Going to get data&&&&&&&&&&&&&&&&&&&&&&&&&&&");
						getIndirectCollaboratorsFromServlet();
