...
public class RegProjectClient {
	...
	public void setConfigProjectValues(String pName, String oName, String ipT,
			String ipSQL, String levelNo, String collabNo) {
		projectName = pName;
		ownerName = oName;
		ipAddTomcat = ipT;
		ipAddMySQL = ipSQL;
	...
	public boolean executeClient() throws Exception {
		...
		if (DEBUG)
			System.out.println("Invoking Servlet RegisterProject" + "http://"
					+ ipAddTomcat
					+ "/collabserver/RegisterProjectServlet?pName="
					+ projectName + "&oName=" + ownerName);
		HttpGet httpget = new HttpGet("http://" + ipAddTomcat
				+ "/collabserver/RegisterProjectServlet?pName=" + projectName
				+ "&oName=" + ownerName + "&levelNumber=" + levelNumber
				+ "&collabNumber=" + collabNumber);
