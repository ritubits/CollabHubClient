...
public enum OpenIndirectCollaboratorModelProvider {
	...
	private OpenIndirectCollaboratorModelProvider() {
		...
		if (collaborators == null)
			collaborators.add("Collaboration Not Started");
	...
	public void getCollaboratorsFromServlet() {
		...
		HttpGet httpget = new HttpGet("http://" + ipAddTomcat
				+ "/collabserver/OInCServlet?cName=" + collabName);
		...
		try {
			...
			if (entity != null) {
				...
				if (len != -1 && len < 2048) {
					...
					System.out
							.println("EntityUtils.toString(entity): from OpenIndirect"
									+ data);
					if (data.equals("null")) {
						dataExists = false;
						...
						projectVector.add(data);

				} else {
					// Stream content out
					System.out
							.println("Received empty string from server: from OpenIndirect");
				}
			}
	...
	private void parse(String msg) {
		...
		if (msg != null) {
			...
			for (int i = 0; i < temp1.length; i++) {
				 ...
				 * System.out.println("data::"+data); // data=null; }
				 */
	...
	public void parseForFourArtifact(String msg) {
		if (msg != null) {
			...
			for (int i = 0; i < temp1.length; i++) {
				...
				System.out
						.println("Data:: after parseForFourArtifact::" + data);
				System.out.println("Content:: after parseForFourArtifact::"
						+ content);
				collaborators.add(data);
			}
			...
			while (true) {
				...
				try {
					...
					if (collabClient != null) {
						collaborators.clear();
						System.out
								.println("Going to get data: from OpenIndirect");
						getCollaboratorsFromServlet();
						// if (ConflictMessagesView.viewer!=null)
						// ConflictMessagesView.viewer.refresh();
					}
