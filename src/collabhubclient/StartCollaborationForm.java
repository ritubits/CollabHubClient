...
public class StartCollaborationForm {
	/*
	 * static boolean DEBUG= false; // static StartCollaborationClient
	 * collabClient = null; static Boolean registerSuceed = false; //static
	 * String regProjectName= null;
	 * 
	 * public StartCollaborationForm() { // collabClient= new
	 * StartCollaborationClient(); }
	 * 
	 * /* public StartCollaborationClient getCollabClient() { //after starting
	 * collaboration //all should use this client return collabClient; }
	 */

	/*
	 * public Boolean getCollabStatus() { return registerSuceed; }
	 * 
	 * public Boolean executeForm() { JFrame frame = new
	 * JFrame("Start Collaboration: Collaborator"); frame.setSize(400, 250);
	 * 
	 ...
	 * collabLabel.setBounds(10, 40, 150, 25); panel.add(collabLabel);
	 * 
	 * final JTextField collabText = new JTextField(20);
	 * collabText.setBounds(200, 40, 160, 25); panel.add(collabText);
	 * 
	 * 
	 * 
	 ...
	 * 
	 * if (DEBUG) System.out.println("In StartCollaborationForm"); if
	 * ((!projectText.getText().isEmpty())) { if (DEBUG)
	 * System.out.println("Project Name:"+ projectText.getText()); if (DEBUG)
	 * System.out.println("Collaborator Name:"+ collabText.getText()); if
	 * (DEBUG) System.out.println("IP Address:"+ serverText.getText());}
	 * 
	 * if ((!projectText.getText().isEmpty()) &&
	 * (!collabText.getText().isEmpty()) && (!serverText.getText().isEmpty()) &&
	 * (!DBserverText.getText().isEmpty())) { //call startCollaborationClient //
	 * update table userdetails_projectName try {
	 * collabClient.setConfigProjectValues(projectText.getText(),
	 * collabText.getText(), serverText.getText(), DBserverText.getText());
	 * boolean done = collabClient.createCollabClient(); // if client created if
	 * (done) { boolean status= collabClient.updateUserTable(); if (status) {
	 * 
	 * // dispose this window registerSuceed= true; regProjectName =
	 * projectText.getText(); JOptionPane.showMessageDialog(null,
	 * "Successfully Connected to CollabHub", "Message Info",
	 * JOptionPane.INFORMATION_MESSAGE);
	 * 
	 * } else JOptionPane.showMessageDialog(null,
	 * "Unable to connect to CollabHub", "Message Info",
	 * JOptionPane.INFORMATION_MESSAGE); } } catch (Exception ex) { if (DEBUG)
	 * System.out.println("Error calling collabClient");
	 * JOptionPane.showMessageDialog(null, "Unable to connect to CollabHub",
	 * "Message Info", JOptionPane.INFORMATION_MESSAGE); ex.printStackTrace(); }
	 * finally { //do not close client here /* try{ if (collabClient != null)
	 * collabClient.closeClient(); } catch (Exception ee){ ee.printStackTrace();
	 * }
	 */
	/*
