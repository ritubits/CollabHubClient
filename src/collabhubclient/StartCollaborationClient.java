...
public class StartCollaborationClient {
	...
	public void setConfigProjectValues(String pName, String cName, String ipT,
			String ipSQL) {
		projectName = pName;
		collabName = cName;
		ipAddTomcat = ipT;
	...
	public boolean createCollabClient() {
		if (DEBUG)
			System.out
					.println("Before creating client: StartCollaborationClient");
		httpclient = HttpClients.createDefault();

		if (DEBUG)
			System.out
					.println("After creating client: StartCollaborationClient");
	...
	public boolean updateUserTable() throws Exception {
		...
		if (DEBUG)
			System.out.println("Invoking Servlet StartCollaboration"
					+ "http://" + ipAddTomcat
					+ "/collabserver/StartCollaborationServlet?pName="
					+ projectName + "&cName=" + collabName);
		...
		if (DEBUG)
			System.out.println("StartCollaborationClient "
		 ...
		 * 
		 * if (entity != null) { long len = entity.getContentLength(); if (len
		 * != -1 && len < 2048) {
		 * projectVector.add(EntityUtils.toString(entity));
		 * 
		 * } else { // Stream content out
		 * System.out.println("Received empty string from server"); } }
		 * 
		 * Enumeration enumVect = projectVector.elements(); while
		 * (enumVect.hasMoreElements()) {
	...
	public boolean getAllowedCollaborators() throws IOException {
		...
		System.out.println("http://" + ipAddTomcat
				+ "/collabserver/AllowedCollaboratorServlet?pName="
				+ projectName + "&cName=" + collabName);
		HttpGet httpget = new HttpGet("http://" + ipAddTomcat
				+ "/collabserver/AllowedCollaboratorServlet?pName="
				+ projectName + "&cName=" + collabName);
