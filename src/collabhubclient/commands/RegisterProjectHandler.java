...
public class RegisterProjectHandler implements IHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		String mySQLIP = Activator.getDefault().getPreferenceStore()
				.getString("mySQLIP");
		String levelNumber = Activator.getDefault().getPreferenceStore()
				.getString("levelNumber");
		String collabNumber = Activator.getDefault().getPreferenceStore()
				.getString("collabNumber");
		...
		if (projectName.equals("null") || ownerName.equals("null")) {
			JOptionPane
					.showMessageDialog(
							null,
							"Project Name or Owner Name is NULL : Set using Windows -> Preferences -> CollabHub Registration Parameters",
							"Message Info", JOptionPane.INFORMATION_MESSAGE);
		} else {
			JOptionPane.showMessageDialog(null,
					"Using Parameters:: Project Name: " + projectName
							+ " Owner Name: " + ownerName, "Message Info",
					JOptionPane.INFORMATION_MESSAGE);

			regObject = new RegProjectClient();
			try {
				regObject.setConfigProjectValues(projectName, ownerName,
						tomcatIP, mySQLIP, levelNumber, collabNumber);
				...
				if (status) {
					...
					JOptionPane.showMessageDialog(null,
							"Project Successfully Registered", "Message Info",
							JOptionPane.INFORMATION_MESSAGE);
