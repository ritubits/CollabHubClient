...
public class StartCollaborationHandler implements IHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		String collabName = Activator.getDefault().getPreferenceStore()
				.getString("collabName");
		String tomcatIP = Activator.getDefault().getPreferenceStore()
				.getString("tomcatIP");
		String mySQLIP = Activator.getDefault().getPreferenceStore()
				.getString("mySQLIP");
		String simulationMode = Activator.getDefault().getPreferenceStore()
				.getString("simulationMode");
		String simulationPath = Activator.getDefault().getPreferenceStore()
				.getString("simulationPath");
		...
		if (projectName.equals("null") || collabName.equals("null")) {
			JOptionPane
					.showMessageDialog(
							null,
							"Project Name or Collaborator Name is NULL : Set using Windows -> Preferences -> CollabHub Connection Parameters",
							"Message Info", JOptionPane.INFORMATION_MESSAGE);
		} else {
			JOptionPane.showMessageDialog(null,
					"Using Parameters:: Project Name: " + projectName
			...
			try {
				...
				collabClient.setConfigProjectValues(projectName, collabName,
						tomcatIP, mySQLIP);
				...
				if (done) {
					...
					if (validCollab) {
						...
						if (status) {
							...
							JOptionPane.showMessageDialog(null,
									"Successfully Connected to CollabHub",
									"Message Info",
							...
							JOptionPane.showMessageDialog(null,
									"Unable to connect to CollabHub",
									"Message Info",
									JOptionPane.INFORMATION_MESSAGE);
						}
					}// validCollab
						...
						JOptionPane
								.showMessageDialog(
										null,
										"You cannot join for collaboration: Allowable limit over",
										"Message Info",
										JOptionPane.INFORMATION_MESSAGE);
					}
				}// if done
				...
				if (done && validCollab && status) {
					...
					provider.getBroker().post(
							CollabEventsConstants.COLLAB_TOPIC_START, argmaps);
				}
		...
		// test live preview
		/*
		 * try{ boolean status= false; if (collabClient!=null) { status=
		 * LivePreviewFunction(collabClient); if (status== true)
		 * OpenNewIDE("DevR"); } }catch (Exception e) { e.printStackTrace(); }
		 */
		...
		if (activePage != null) {
			...
			if (editor != null) {
				...
				if (input instanceof IFileEditorInput) {
					...
					File file = new File("/temp/" + collabName
							+ "_artifact.txt");
	...
	public boolean LivePreviewFunction(StartCollaborationClient collabClient)
			throws Exception {
		// invoke Servlet

		CloseableHttpClient httpclient = StartCollaborationClient.httpclient;
		HttpGet httpget = new HttpGet("http://" + "localhost:8080"
				+ "/collabserver/LivePreviewServlet?&cName=" + "DevR");
		if (DEBUG)
			System.out.println("Invoking Servlet:: " + "http://"
					+ "localhost:8080"
					+ "/collabserver/LivePreviewServlet?&cName=" + "DevR");
		...
		if (DEBUG)
			System.out.println("CollabUserActivityClient "
		...
		while (enumVect.hasMoreElements()) {
			...
			if (DEBUG)
				System.out.println("From LivePreview servlet: " + fileContent);
			// write to file at temp/collabName_artifact.java

			try {
				FileWriter writer = new FileWriter("/temp/" + "DevR"
						+ "_artifact.txt", false);
