...
public class EntryFormDeRegProject {
	...
	/*
	 * public EntryFormDeRegProject() { deRegObject= new DeRegProjectClient(); }
	 * 
	 * public void executeForm() { JFrame frame = new
	 * JFrame("DeRegister Project: Admin"); frame.setSize(400, 250);
	 * 
	 ...
	 * 
	 * if (DEBUG) System.out.println("In deregisterProjectForm: DERegister"); if
	 ...
	 * (!DBserverText.getText().isEmpty())) { // call the deregHttpClient here-
	 * which calls the DeRegProjectServlet //which creates table for the
	 * regProject_projectName // send the parameters ... // write to file in the
	 * servlet try { deRegObject.setConfigProjectValues(projectText.getText(),
	 * ownerText.getText(), serverText.getText(), DBserverText.getText());
	 * boolean status= deRegObject.executeClient(); if (status) { // give
	 * message written to config file // dispose this window
	 * JOptionPane.showMessageDialog(null, "Project Successfully DeRegistered",
	 ...
	 * JOptionPane.showMessageDialog(null, "Unable to deregister the project",
	 * "Message Info", JOptionPane.INFORMATION_MESSAGE); ex.printStackTrace(); }
	 * finally { //close client here try{ if (deRegObject != null)
	 * deRegObject.closeClient(); } catch (Exception ee){ ee.printStackTrace();
	 * }
	 * 
